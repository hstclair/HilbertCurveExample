...
public class HilbertCurveImpl implements HilbertCurve {
    ...
    /** the GrayCode instance used to convert each group of distance bits to its corresponding Gray code value */
    public final GrayCode grayCode;
    ...
    public long[] distanceToCoords(long distance) {
        ...
        for (int order = 0; order < steps; order++) {
            ...
            long originalBitVector = grayCode.encode(vertexRank);
    ...
    public long coordsToDistance(long[] coords) {
        ...
        if (coords.length != dimension)
            throw new IllegalArgumentException("coords.length not equal to dimension");
        ...
        for (int order = 0; order < steps; order++) {
            ...
            long vertexRank = grayCode.decode(originalBitVector);
